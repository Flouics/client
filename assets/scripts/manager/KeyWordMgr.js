 ...
 */
var KeyWordMgr = function () {
};

KeyWordMgr.prototype.init = function () {
    //敏感字符
    this.sensitiveWordMap = {};
};

KeyWordMgr.prototype.isContainsSensitiveWord = function (s) {
    if (s.length < 1) {
        return false;
    }
    if (!this.sensitiveWordMap) {
        this.init();
    }
    s = s.toLowerCase();
...
//表情检测
KeyWordMgr.prototype.isEmoji = function (s) {
    if (s.length < 1) {
        return false;
    }
    // "\ud83c[\udc00-\udfff]|\ud83d[\udc00-\udfff]"为emoji表情
...
//不支持的字符
KeyWordMgr.prototype.isNonSupport = function (s) {
    if (s.length < 1) {
        return false;
    }
...
KeyWordMgr.prototype.replaceKeyWord = function (s, replace_str) {
...
//通用字符串检查。
KeyWordMgr.prototype.isStringValid = function (_string) {
    var errorCode = 0;
    //防注入。
    var reg = /[\(\)\[\]\{\}\^\|\?\+\.'";,]/;
    if (reg.test(_string)) {
