namespace toolKit {
    export class ToolKit {
        ...
        ArrayGroup(array: any[], count_num: number) {
            var loop_get: any(list: any[], count: number, index: number) {
                if (count < count_num) {
                    var list_ret = [], ret;
         ...
         */
        getHandScale(picSize, framSize, scale) {
        ...
        //label上数值累加到目标值
        labelAccumulator(label, formateStr, startValue, finalValue, delta, cb) {
            if (!delta || delta == 0) {
                if (delta) {
        ...
        //加载图片
        loadResSpritFrame(res_url, spt, cb) {
            if (!res_url) return;
        ...
        //加载图集
        loadResSpriteAtlas(res_url, name, spt, cb) {
            if (!res_url) return;
        ...
        copyTextToClipboardBrower_1(content) {
            var success = true;
        ...
        copyTextToClipboardBrower_2(content) {
            var success = true;
